...
namespace mime {
    ...
    mime_cache *mime_cache_reader::get_actual_cache() {
        ...
        auto cache_exists = helpers::file::is_exists(helpers::environment::get_mime_cache_file_path());
        if (helpers::file::is_exists(helpers::environment::get_mime_cache_file_path())) {
            last_modification_date =
                    helpers::file::get_last_modification_date(helpers::environment::get_mime_cache_file_path());
            ...
            if (!m_mime_cache || m_mime_cache_last_modification_date < last_modification_date) {
                ...
                m_mime_cache = mime::mime_cache_parser::parse_file(helpers::environment::get_mime_cache_file_path());
            }
        } else {
            if (helpers::file::is_exists(helpers::environment::get_distribution_defaults_apps_path())) {
                pathes.emplace_back(helpers::environment::get_distribution_defaults_apps_path());
            }
            if (helpers::file::is_exists(helpers::environment::get_distribution_mime_apps_path())) {
                pathes.emplace_back(helpers::environment::get_distribution_mime_apps_path());
            }
        }
