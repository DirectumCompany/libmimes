...
void check_associations(const std::string &example_file_name,
    ...
    auto registered_associations = mime::parser::parse_file(example_file_name);
...
TEST(parser__parse, only_defaults) {
...
TEST(parser__parse, only_removed) {
    auto current_directory = helpers::path::get_current_working_directory();
